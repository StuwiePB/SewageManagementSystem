...
class StatisticsController extends Controller
{
    ...
    {
        return view('r_operators.statistics.index');
    }
    ...
    public function show(Request $request)
    {
        ...
        return view('r_operators.statistics.show', compact('stats', 'type', 'start', 'end'));
    }
    ...
    protected function computeStats(string $type, CarbonInterface $start, CarbonInterface $end): array
    {
        ...
        return match ($type) {
            self::TYPE_REPORTS => array_merge($base, $this->reportStats($start, $end)),
            self::TYPE_WORK_ORDERS => array_merge($base, $this->workOrderStats($start, $end)),
            self::TYPE_GEOGRAPHIC => array_merge($base, $this->geographicStats($start, $end)),
            default => $base,
        };
    }
    ...
    {
        $total = OperationsReport::whereBetween('created_at', [$start, $end])->count();
    ...
    {
        $total = WorkOrder::whereBetween('created_at', [$start, $end])->count();
        $completed = WorkOrder::whereBetween('created_at', [$start, $end])->where('status', 'completed')->count();
        ...
        $pendingBacklog = WorkOrder::whereIn('status', ['pending', 'assigned', 'in_progress', 'on_the_way', 'on_site'])->count();
    ...
    public function exportCsv(Request $request)
    {
        ...
        $filename = 'brudms-ops-statistics-'.$type.'-'.$start->format('Y-m-d').'-to-'.$end->format('Y-m-d').'.csv';

        return response()->streamDownload(
            function () use ($stats, $type) {
                $out = fopen('php://output', 'w');
                fputcsv($out, ['BruDMS Statistics', $stats['title'] ?? $type]);
    ...
    public function exportPdf(Request $request)
    {
        ...
        return view('r_operators.statistics.pdf', compact('stats', 'type', 'start', 'end'));
    }
}
