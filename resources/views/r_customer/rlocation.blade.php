<x-layouts::customer :title="__('Report Problem') . ' – BruDMS'" :bare="true">
    ...
    <button type="button" id="confirm-location-btn" class="press-btn" style="position: fixed; left: 50%; transform: translateX(-50%); bottom: 24px; z-index: 7; width: 82%; max-width: 360px; height: 48px; border: none; border-radius: 16px; background: #04BCFF; color: #0a1628; font-family: Poppins, sans-serif; font-weight: 700; font-size: 15px; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(4, 188, 255, 0.3);">Confirm location</button>
    ...
    <script>
        ...
        function updateDisplay(lat, lng) {
            ...
            setTimeout(function() {
                fetch('https://nominatim.openstreetmap.org/reverse?lat=' + lat + '&lon=' + lng + '&format=json', {
                    ...
                    try { sessionStorage.setItem('rproblem_address', addr); } catch (e) {}
                }).catch(function() {
                    el.textContent = lat.toFixed(6) + ', ' + lng.toFixed(6);
                    el.style.color = 'rgba(255, 255, 255, 0.95)';
                    try { sessionStorage.removeItem('rproblem_address'); } catch (e) {}
                });
            }, delay);
        }

        function setLocation(lat, lng) {
            try { sessionStorage.setItem('rproblem_lat', String(lat)); sessionStorage.setItem('rproblem_lng', String(lng)); } catch (e) {}
        ...
        document.getElementById('confirm-location-btn').addEventListener('click', function() {
            ...
            try { sessionStorage.setItem('rproblem_lat', String(latlng.lat)); sessionStorage.setItem('rproblem_lng', String(latlng.lng)); } catch (e) {}
