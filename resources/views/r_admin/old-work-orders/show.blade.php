...
@section('title', 'Old Work Order '.$archiveWorkOrder->work_order_number)

@section('content')
<div class="header">
    <div>
        <a href="{{ route('admin.old-work-orders.index') }}" class="back-link">← Old Work Orders</a>
        <h1 style="margin-top: 0.75rem;">{{ $archiveWorkOrder->work_order_number }}</h1>
        ...
        <div class="detail-row">
            ...
            <p class="info-value">
                @if($archiveWorkOrder->paperReport)
                    <a href="{{ route('admin.old-reports.show', $archiveWorkOrder->paperReport) }}" class="link-primary">{{ $archiveWorkOrder->paperReport->archive_number }}</a>
                @else
                    —
                @endif
            </p>
